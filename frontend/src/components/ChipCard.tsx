...
export const ChipCard: React.FC<ChipCardProps> = ({
  ...
  const totalPotMoney = isValueMode ? (totalChips + totalLoansMoney) : ((totalChips * chipValue) + totalLoansMoney);
  const inPlayMoney = isValueMode ? playerChips : (playerChips * chipValue);
  const bankMoney = isValueMode ? bankChips : (bankChips * chipValue);
    ...
    <View style={styles.card}>
      ...
      <View style={styles.headerRow}>
        <View style={styles.badgeRow}>
          ...
          <Text style={styles.badgeLabel}>{isValueMode ? 'VAULT & POT BALANCE' : 'CHIP INVENTORY'}</Text>
        </View>
        {isValueMode ? (
          <View style={styles.ratePill}>
            <Text style={styles.ratePillText}>By Value Mode</Text>
          </View>
      ...
      <View style={styles.metricRow}>
        ...
        <View style={styles.metricCol}>
          ...
          {isValueMode ? (
            <>
              <Text style={[styles.metricChips, { color: colors.primary }]}>₹{inPlayMoney.toLocaleString('en-IN')}</Text>
              <Text style={styles.chipUnit}>In-hand</Text>
            </>
          ) : (
            <>
          ...
          {isValueMode ? (
            <>
              <Text style={[styles.metricChips, bankChips === 0 && { color: colors.warningText }]}>
                ₹{bankMoney.toLocaleString('en-IN')}
              </Text>
              <Text style={styles.chipUnit}>Available</Text>
            </>
          ) : (
            <>
              <Text style={[styles.metricChips, bankChips === 0 && { color: colors.warningText }]}>
        ...
        <View style={styles.metricCol}>
          ...
          {isValueMode ? (
            <>
              <Text style={[styles.metricChips, { color: colors.chipGold }]}>₹{totalPotMoney.toLocaleString('en-IN')}</Text>
              {hasLoans ? (
                <Text style={styles.loansTag}>+₹{totalLoansMoney.toLocaleString('en-IN')} shots</Text>
              ) : (
                <Text style={[styles.chipUnit, { color: colors.chipGold }]}>Total Pot</Text>
              )}
            </>
          ) : (
            <>
